...
export default function Archive() {
  ...
  const renderCell = useCallback((key, item) => {
    switch (key) {
      case 'orderId': return item.orderId;
      case 'productName': return item.productName;
      case 'status': return <span className="px-2 py-0.5 rounded text-xs bg-green-100 text-green-700">Payment Received</span>;
      case 'platformName': return platformMap[item.platformId] || item.platformId;
      case 'mediatorName': return mediatorMap[item.mediatorId] || item.mediatorId;
      case 'orderedDate': return item.orderedDate ? formatDate(item.orderedDate) : (item.paymentReceivedDate ? formatDate(item.paymentReceivedDate) : '-');
      case 'amountRupees': return formatCurrency(item.amountRupees);
      case 'refundAmountRupees': return formatCurrency(item.refundAmountRupees);
      default: return item[key];
    }
  }, [platformMap, mediatorMap]);
          ...
          <div className="md:hidden space-y-3 mb-4">
            {items.map(i => (
              <ReviewCard
                ...
                onEdit={() => { }} // No edit in archive for now or redirect
                onDelete={() => { }}
              />
            ))}
            {items.length === 0 && <div className="text-center py-8 text-gray-500">No archived reviews.</div>}
          </div>
