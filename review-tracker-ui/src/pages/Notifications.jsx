...
import { BellIcon, ExclamationTriangleIcon, InformationCircleIcon, ClockIcon, ChevronRightIcon, Cog6ToothIcon, BookOpenIcon } from "@heroicons/react/24/outline";
...
export default function Notifications() {
    ...
    return (
        <div className="max-w-3xl mx-auto pb-20">
                ...
                {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                        <button
                            ...
                            className="w-full text-left p-4 hover:bg-gray-50 transition-colors flex gap-4 items-start group first:rounded-t-none last:rounded-b-xl"
                        >
                            <div className={`p-2 rounded-full shrink-0 ${n.type === 'URGENT' ? 'bg-red-100 text-red-600' :
                            ...
                            <div className="flex-1 min-w-0 pt-1">
                                <div className="flex items-center justify-between mb-1">
                                    ...
                                    <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 whitespace-nowrap">Order: {n.orderId}</span>
                                </div>
            ...
            <Modal open={rulesOpen} title="Logic Rules" onClose={() => setRulesOpen(false)}>
                <div className="space-y-4 text-sm text-gray-600">
                    <p>The system automatically generates notifications based on the following logic:</p>
                    <div className="grid gap-3">
                        <div className="p-3 rounded-lg bg-gray-50 border border-gray-100">
                            <span className="font-bold block text-gray-900 mb-1">Review Reminder</span>
                            7 days after ordering if no review date is set.
                        </div>
                        <div className="p-3 rounded-lg bg-gray-50 border border-gray-100">
                            <span className="font-bold block text-gray-900 mb-1">Refund Form</span>
                            3 days after review acceptance if form not submitted.
                        </div>
                        <div className="p-3 rounded-lg bg-gray-50 border border-gray-100">
                            <span className="font-bold block text-gray-900 mb-1">Payment Check</span>
                            45 days after refund form submission for status check.
                        </div>
                        <div className="p-3 rounded-lg bg-gray-50 border border-gray-100">
                            <span className="font-bold block text-gray-900 mb-1">Crucial Overdue</span>
                            60 days after refund form submission if still pending.
                        </div>
                    </div>
                </div>
                <div className="mt-6 flex justify-end">
                    <button onClick={() => setRulesOpen(false)} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-medium text-sm">Close</button>
                </div>
            </Modal>
        </div>
    );
